...
from uatrpo.common.initializers import init_seeds
from uatrpo.common.logger import Logger
...
class BaseAlg:
    ...
    def _ac_setup(self):
        ...
        # TV vs. KL: delta_kl = (eps_tv)**2 / 2
        if self.ac_kwargs['eps_tv']:
            self.eps_tv = self.ac_kwargs['eps_tv']
        else:
            delta_kl = self.ac_kwargs['delta_kl']
            self.eps_tv = np.sqrt(2 * delta_kl)
        self.eps = self.eps_tv * self.ac_kwargs['eps_mult']
        self.trust_vary = self.ac_kwargs['trust_vary']
